...
model.fit(X, Y / 6, batch_size=500, epochs=1000, validation_split=0.1, verbose=2, callbacks=callbacks)
...
weights_prop = (np.abs(weights_in) @ np.abs(weights_hidden)) / weights_in.shape[1]

# Looking at weights
print('Weights:')
weights_in = np.sum(np.abs(np.asarray(model.weights[0].container.data)), axis=1)
bias = np.asarray(model.weights[len(model.weights) - 1].container.data)

print('Bias: {}'.format(bias[0]))
fweights = []
for a in range(n_features):
    weight = weights_prop[a][0]
    fweights += [(a, weight)]
